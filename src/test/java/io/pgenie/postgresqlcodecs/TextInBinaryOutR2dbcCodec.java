 ...
 * {@link io.r2dbc.postgresql.codec.Codec} SPI using <b>text format for parameter
 * encoding</b> and <b>binary format for result decoding</b>.
 *
 * <p>Use this with a connection that has {@code forceBinary} enabled so that the
 * server returns columns in binary format.
 */
public class TextInBinaryOutR2dbcCodec<A> implements io.r2dbc.postgresql.codec.Codec<A> {
    ...
    public TextInBinaryOutR2dbcCodec(io.pgenie.postgresqlcodecs.codecs.Codec<A> codec, Class<A> type) {
        this.codec = codec;
        this.type = type;
    }
